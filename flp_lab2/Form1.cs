...
namespace flp_lab2
{
    public partial class Form1 : Form
    {
        ...
        private void ReplacePlaceholdersImproved(WordprocessingDocument wordDoc)
        {
            ...
            var apps = GetApplicationsFromGrid();
            AddAppendicesToDocument(wordDoc, apps);
        }
        ...
        private void ReplaceTextInBody(OpenXmlElement element, Dictionary<string, string> replacements)
        {
            ...
            foreach (var paragraph in paragraphs)
            {
                ...
                if (textElements.Count > 0 && textElements[0].Parent is Run firstRun && firstRun.RunProperties != null)
                {
                    runProperties = (RunProperties)firstRun.RunProperties.Clone();
                }
                ...
                for (int i = 0; i < lines.Length; i++)
                {
                    ...
                    {
                        newRun.RunProperties = (RunProperties)runProperties.Clone();
                    }

                    var newTextElement = new Text(line);
        ...
        private List<ApplicationInfo> GetApplicationsFromGrid()
        {
            ...
            foreach (DataGridViewRow row in dgvApps.Rows)
            {
                ...
                string title = row.Cells["AppTitle"].Value?.ToString();
                string text = row.Cells["AppText"].Value?.ToString();
        ...
        private string GenerateAppendixListText(List<ApplicationInfo> apps)
        {
            ...
            {
                result += $"{i + 1}. {apps[i].Title}\n";
            }

            return result;
        }

        private void AddAppendicesToDocument(WordprocessingDocument wordDoc, List<ApplicationInfo> apps)
        {
            if (apps.Count == 0) return;
            ...
            for (int i = 0; i < apps.Count; i++)
            {
                ...
                string appNumber = (apps.Count == 1) ? "Приложение" : $"Приложение {i + 1}";
                ...
                Paragraph appLabelPara = new Paragraph(
                    new ParagraphProperties(new Justification() { Val = JustificationValues.Right }),
                    new Run(new Text(appNumber))
                );
                body.Append(appLabelPara);

                if (!string.IsNullOrWhiteSpace(app.Title))
                {
                    Paragraph titlePara = new Paragraph(
                        new ParagraphProperties(new Justification() { Val = JustificationValues.Center }),
                        new Run(new Text(app.Title))
                    );
                ...
                {
                    string[] paragraphs = app.Text.Split(new[] { "\r\n", "\n" }, StringSplitOptions.None);
                    foreach (string paraText in paragraphs)
                    {
                        if (!string.IsNullOrWhiteSpace(paraText))
                        {
                            Paragraph textPara = new Paragraph(new Run(new Text(paraText)));
        ...
        {
            var apps = GetApplicationsFromGrid();
            string appendixList = GenerateAppendixListText(apps);
            ...
            return new Dictionary<string, string>
            {
                ...
                { "{APPENDICES_LIST}", appendixList },
                { "{APPENDICES_COUNT}", apps.Count.ToString() }
            };
        }
        ...
        private void btnDefault_Click(object sender, EventArgs e)
        {
            ...
            dgvApps.Rows.Clear();
            dgvApps.Rows.Add("Причина отмены лабораторной", "Обоснование причин, по которым лабораторная работа была отменена...");
            dgvApps.Rows.Add("Почему всё плохо", "Детальный анализ проблем и их последствий...");
        }
    }
